...
//// Rujia Liu
//// This code implements the simpliest yet efficient-enough algorithm I'm aware
//// of
//// Readers are encouraged to experiment on other algorithms (especially for
//// better efficiency!)
//#include <cctype>
//#include <cstdio>
//#include <cstring>
...
//        id[maxs][maxs]; // cnt is the number of non-wall cells
//    cnt = 0;
//    for (int i = 0; i < h; i++)
//      for (int j = 0; j < w; j++)
//        if (maze[i][j] != '#') {
//          x[cnt] = i; // 行号
//          y[cnt] = j;  // 列号
//          id[i][j] = cnt; // cnt is the number of non-wall cells
//          if (islower(maze[i][j]))
//            s[maze[i][j] - 'a'] = cnt; // // starting/ending position of each ghost
...
//        // "Outermost cells of a map are walls" means we don't need to check
//        // out-of-bound
//        if (maze[nx][ny] != '#')
//          G[i][deg[i]++] = id[nx][ny];// cnt is the number of non-wall cells before (nx,ny)
//      }
//    }
//
//    // add fakes nodes so that in each case we have 3 ghosts. this makes the
//    // code shorter
//    if (n <= 2) {
//      deg[cnt] = 1;
//      G[cnt][0] = cnt;
